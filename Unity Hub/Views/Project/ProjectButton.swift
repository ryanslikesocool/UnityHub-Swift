...
struct ProjectButton: View {
	...
	var body: some View {
		...
		.swipeActions(edge: .leading, allowsFullSwipe: true) {
			Button(project.pinned ? "􀎦 Unpin" : "􀎧 Pin") {
				togglePin()
			}
			.tint(.orange)
		}
	...
	func dropDownMenu() -> some View {
		Group {
			Button("􀈖 Reveal in Finder") { NSWorkspace.shared.selectFile(nil, inFileViewerRootedAtPath: project.path) }
			if settings.hub.useEmoji {
				Divider()
				Button("􀙌 Select Emoji") { showPopover.toggle() }
			...
			if settings.hub.usePins {
				...
				Button(project.pinned ? "􀎦 Unpin" : "􀎧 Pin") { togglePin() }
				Divider()
			}
			Button("􀈒 Remove Project", action: { deleteAction(project) })
		}
	}
